...
def _is_generated_docs_data(rel: str) -> bool:
    return rel.startswith("docs/public/static/data/") and rel.endswith(".json")
...
def _tokenize(text: str) -> List[str]:
    # Keep short identifiers and common separators (Expo docs + source files).
...
def _score_match(rel: str, text: str, text_lower: str, query: str, query_lower: str) -> Tuple[float, int, int]:
    ...
    if query_tokens:
        ...
        for token in query_tokens:
            ...
            if token_count > 0:
                # Saturating TF weight to avoid huge files dominating ranking.
...
def _tool_definitions() -> List[Dict[str, Any]]:
    return [
        ...
        {
            "name": "search_docs",
            "description": "Search Expo knowledge files by path and content with semantic-lite relevance scoring and pagination.",
            "inputSchema": {
                "type": "object",
                "properties": {
...
def _tool_search_docs(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for rel, doc in _DOCS_INDEX.items():
        rel_lower = rel.lower()
        generated_data_file = _is_generated_docs_data(rel)
        ...
        if not phrase_path_hit and not phrase_content_hit and token_hit_total < min_token_hits:
            continue
        if recall_mode == "high_precision" and generated_data_file:
            # In precision mode, keep generated API dumps out of ranking.
            continue
        ...
        if ranking_profile == "balanced":
            # Reduce path-heavy bias for broader code searches.
        ...
        if content_hit:
            ...
            if snippet_idx < 0 and query_tokens:
                ...
                token_positions = [p for p in token_positions if p >= 0]
                if token_positions:
        ...
        results.append(
            {
                ...
                "generatedData": generated_data_file,
                "snippet": snippet,
                "score": score,
                "_pathIdx": path_idx,
    ...
    if method == "initialize":
        return _ok_response(
            req_id,
            {
                ...
                "serverInfo": {"name": "expo-knowledge-mcp", "version": "2.1.0"},
            },
        )
